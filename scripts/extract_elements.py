...
def run(project_dir):
    ...
    prov = get_provider(spec.get("provider"), spec.get("provider_config"))
    for el in spec["elements"]:
        ...
        if mode == "cutout":
            raise RuntimeError("Liblib workflow backend does not provide background removal. Use a pre-cut transparent PNG and set mode to 'crop'.")
        else:
            elem = crop
        if el.get("erase"):
